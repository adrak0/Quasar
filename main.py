...
async def on_ready():
    await bot.change_presence(activity=discord.Streaming(
        name='a song ❄️🔥', url='https://www.youtube.com/watch?v=bXyenLi--1M'))
    print("Bot is ready!")
...
@bot.slash_command(name="weather",
                   description="☁️ Forecast for a specific location.")
...
async def weather(ctx, location):
    try:
        ...
        async with aiohttp.ClientSession() as cs:
            async with cs.get(
                    f"https://api.openweathermap.org/data/2.5/weather?q={location}&appid={wapi}&units=metric"
            ) as r:
                data = await r.json()
                embed = discord.Embed(
                ...
                embed.add_field(name="Temperatue",
                ...
                embed.add_field(name="Pressure",
                                value=f"{data['main']['pressure']} Pa",
                                inline=False)
                embed.add_field(name="Sunrise",
                                value=f"<t:{data['sys']['sunrise']}:R>",
                                inline=True)
                embed.add_field(name="Sunset",
                                value=f"<t:{data['sys']['sunset']}:R>",
                                inline=True)
                await ctx.respond(embed=embed)
    except KeyError:
        await ctx.respond("`That location doesn't exist in our database.`",
                          ephemeral=True)
    ...
    for i in range(len(qrObj["results"])):
        if not i > 9:
            embed.add_field(
                ...
                value=
                f"Launch window: <t:{parser.parse(qrObj['results'][i]['window_start']).strftime('%s')}:R>",
                inline=False)
    await ctx.respond(embed=embed)
